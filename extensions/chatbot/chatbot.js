...
async function processChatMessage(chatdata)
{
    ...
    try
    {
        ...
        if(chatdata.message.startsWith("Hey Marvin") && serverConfig.chatbotenabled === "on")
        {

            let question = chatdata.message.substring((chatdata.message.indexOf("Marvin") + 7))
            ...
            if(enableOpenAI)
            {
                ...
                if(response.data.choices[0].text.startsWith("?"))
                {
                    ...
                    postMessageToTwitch(response.data.choices[0].text)
            }
            else{
